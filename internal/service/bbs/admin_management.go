...
func (s *Service) AdminPlugins(ctx context.Context) ([]view.AdminPlugin, error) {
	root := filepath.Join(s.phpRoot(ctx), "plugin")
	entries, err := os.ReadDir(root)
	if err != nil {
		if os.IsNotExist(err) {
			return []view.AdminPlugin{}, nil
		}
		return nil, gerror.Wrap(err, "读取插件目录失败")
	}
	plugins := make([]view.AdminPlugin, 0, len(entries))
	for _, entry := range entries {
		if !entry.IsDir() {
			continue
		}
		confPath := filepath.Join(root, entry.Name(), "conf.json")
		...
		plugin := view.AdminPlugin{
			Dir: entry.Name(), Name: pluginText(conf["name"]), Brief: pluginText(conf["brief"]),
			Version: pluginVersion(conf["version"]), Installed: pluginBool(conf["installed"]), Enabled: pluginBool(conf["enable"]),
			HasConfig: fileExists(filepath.Join(root, entry.Name(), "setting.php")),
		}
		if plugin.Name == "" {
...
func (s *Service) SetAdminPluginState(ctx context.Context, dir, action string) error {
	if !regexp.MustCompile(`^\w+$`).MatchString(dir) {
		return gerror.New("插件目录格式错误")
	}
	path := filepath.Join(s.phpRoot(ctx), "plugin", dir, "conf.json")
	content, err := os.ReadFile(path)
	if err != nil {
		return gerror.Wrap(err, "插件不存在")
	}
	var conf map[string]any
	if err = json.Unmarshal(content, &conf); err != nil {
		return gerror.Wrap(err, "插件配置格式错误")
	}
	pluginRoot := filepath.Dir(filepath.Dir(path))
	allConfigs, err := readPluginConfigs(pluginRoot)
	if err != nil {
		return err
	}
	switch action {
	case "install", "enable":
		if err = requirePluginDependencies(dir, conf, allConfigs); err != nil {
			return err
		}
		if action == "install" {
			conf["installed"] = 1
		...
		conf["enable"] = 1
	case "unstall":
		if err = requireNoPluginDependents(dir, allConfigs); err != nil {
			return err
		}
		conf["installed"], conf["enable"] = 0, 0
	case "disable":
		if err = requireNoPluginDependents(dir, allConfigs); err != nil {
			return err
		}
		conf["enable"] = 0
	default:
		return gerror.New("未知插件操作")
	}
	encoded, err := json.MarshalIndent(conf, "", "    ")
	if err != nil {
		return gerror.Wrap(err, "生成插件配置失败")
	}
	encoded = append(encoded, '\n')
	if err = writeFileAtomic(path, encoded); err != nil {
		return gerror.Wrap(err, "保存插件配置失败")
	}
	if action == "install" {
		if err = s.disableConflictingPlugins(pluginRoot, dir, allConfigs); err != nil {
			return err
		}
	}
	return s.ClearOriginalCache(ctx, false, true)
}

func (s *Service) disableConflictingPlugins(root, dir string, configs map[string]map[string]any) error {
	targetSuffix := ""
	if index := strings.IndexByte(dir, '_'); index >= 0 {
		targetSuffix = dir[index:]
	}
	for otherDir, conf := range configs {
		if otherDir == dir {
			continue
		}
		conflicts := strings.Contains(dir, "_theme_") && strings.Contains(otherDir, "_theme_")
		if !conflicts && targetSuffix != "" {
			if index := strings.IndexByte(otherDir, '_'); index >= 0 {
				conflicts = otherDir[index:] == targetSuffix
			}
		}
		if !conflicts {
			continue
		...
		conf["installed"], conf["enable"] = 0, 0
		if err := writePluginConfig(filepath.Join(root, otherDir, "conf.json"), conf); err != nil {
			return err
		}
	}
	return nil
...
func pluginText(value any) string {
	switch typed := value.(type) {
	case string:
		return typed
	case map[string]any:
		for _, key := range []string{"zh-cn", "zh_cn", "en-us", "en_us"} {
...
func pluginBool(value any) bool {
	switch typed := value.(type) {
	case bool:
		return typed
	case float64:
...
func pluginVersion(value any) string {
	if value == nil {
		return "1.0"
	}
...
func readPluginConfigs(root string) (map[string]map[string]any, error) {
	entries, err := os.ReadDir(root)
	if err != nil {
		return nil, gerror.Wrap(err, "读取插件目录失败")
	}
	...
	for _, entry := range entries {
		...
		content, readErr := os.ReadFile(filepath.Join(root, entry.Name(), "conf.json"))
		if readErr != nil {
			continue
		}
		var conf map[string]any
		if json.Unmarshal(content, &conf) == nil {
			configs[entry.Name()] = conf
		}
	}
	return configs, nil
}

func requirePluginDependencies(dir string, conf map[string]any, configs map[string]map[string]any) error {
	dependencies, _ := conf["dependencies"].(map[string]any)
	missing := make([]string, 0)
	for dependency := range dependencies {
		dependencyConfig, exists := configs[dependency]
		if !exists || !pluginBool(dependencyConfig["enable"]) {
			missing = append(missing, dependency)
		}
	}
	if len(missing) != 0 {
		sort.Strings(missing)
		return gerror.Newf("插件 %s 依赖以下已启用插件：%s", dir, strings.Join(missing, ", "))
	}
	return nil
}

func requireNoPluginDependents(dir string, configs map[string]map[string]any) error {
	dependents := make([]string, 0)
	for otherDir, conf := range configs {
		if otherDir == dir || !pluginBool(conf["enable"]) {
			continue
		}
		dependencies, _ := conf["dependencies"].(map[string]any)
		if _, depends := dependencies[dir]; depends {
			dependents = append(dependents, otherDir)
		}
	}
	if len(dependents) != 0 {
		sort.Strings(dependents)
		return gerror.Newf("插件正在被依赖，不能停用：%s", strings.Join(dependents, ", "))
	}
	return nil
}

func writePluginConfig(path string, conf map[string]any) error {
	encoded, err := json.MarshalIndent(conf, "", "    ")
	if err != nil {
		return gerror.Wrap(err, "生成插件配置失败")
	}
